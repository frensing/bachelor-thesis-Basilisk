...
The next sections explain the idea of the three main services of the Basilisk platform, namely \acl{hcs} (section \ref{sec:hooks_checking_service}), \acl{jms} (section \ref{sec:jobs_managing_service}), and \acl{tbs} (section \ref{sec:ts_benchmarking_service}).
...
This is based on code review and analysis, and information and diagrams provided by former developers of the project.
\\
...
Lastly the \ac{jms} manages the status for running and pending jobs sent to the \acl{tbs}.
\\
...
Repository events are received from the \ac{hcs} and benchmark job events are send to the \acl{tbs} over the \ac{bjq}.
\\
